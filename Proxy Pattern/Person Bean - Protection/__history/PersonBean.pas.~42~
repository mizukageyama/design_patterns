...
function GetNonOwnerProxy(PersonBean: TPersonBean): TPersonBean;
begin
  var PersonBeanObj := TPersonBean.Create;
  PersonBeanObj.Assign(PersonBean);

  var VMI := TVirtualMethodInterceptor.Create(PersonBeanObj.ClassType);
  VMI.OnBefore := procedure(Instance: TObject; Method: TRttiMethod;
    const Args: TArray<TValue>; out DoInvoke: Boolean; out Result: TValue)
  begin
      if Method.Name.StartsWith('Set') and
        (Method.Name <> 'SetHotOrNotRating') then
      begin
